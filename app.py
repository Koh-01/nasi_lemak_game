    ...
    if action_type == 'pack_standard':
        ...
        for opp in g.players:
            if opp["name"] != p["name"] and opp["hand"]:
                ...
                stolen_count += 1
        p["hand"].sort()
        g.spend_move(f"【{p['name']}】 派小偷摸走了所有人各 1 张手牌")
        ...
        if target_p["flies"][card_idx]: g.log.append("❌ 这包椰浆饭已经有苍蝇了！")
            ...
            g.spend_move(f"【{p['name']}】 把一只苍蝇放到了 【{target_p['name']}】 的第 {card_idx + 1} 包椰浆饭上")
        ...
        g.spend_move(f"【{p['name']}】 用苍蝇拍打死了自己第 {card_idx + 1} 包饭上的苍蝇")
        ...
        if assigned_idx == -1: g.log.append(f"❌ 对方全部长满苍蝇了，扇不过去！")
            ...
            g.spend_move(f"【{p['name']}】 用扇子把苍蝇吹到了 【{target_p['name']}】 的第 {assigned_idx + 1} 包饭上")
